...
export const profileService = {
   ...
   */
  async uploadImage(file: File, folder: "avatars" | "covers" | "projects" | "photos"): Promise<string> {
    ...
    if (isFirebaseConfigured && storage && auth?.currentUser) {
      try {
        ...
        const storageRef = ref(storage, `${folder}/${auth.currentUser.uid}_${Date.now()}_${Math.random().toString(36).substring(2, 7)}.${fileExt}`);
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (e) => {
        ...
        img.onload = () => {
          ...
          const maxDim = folder === "covers" ? 1400 : folder === "photos" ? 1200 : 600;
   ...
   */
  async uploadMultipleImages(files: File[], folder: "photos" = "photos"): Promise<string[]> {
